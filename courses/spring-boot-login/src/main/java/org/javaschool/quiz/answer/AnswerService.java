...
public class AnswerService {
    ...
    @Transactional
    public AnswerResult submit(SubmitAnswerRequest request, StudyMember member) {
        ...
        AnswerAttempt attempt = AnswerAttempt.record(question, member, request.choiceId());
    ...
    // 한 사람이 남긴 풀이 행만 세어 그 사람의 제출 수, 정답 수와 정답률을 만듭니다.
    public QuizStats findStatsOf(StudyMember member) {
        long answeredCount = answerAttemptRepository.countByMember(member);
        long correctCount = answerAttemptRepository.countByMemberAndCorrectTrue(member);
